...
function onBtnClick(e) {
    e.preventDefault();
    let { delay: delay1 , step , amount  } = Object.fromEntries(new FormData(e.target));
    delay1 = Number(delay1);
    step = Number(step);
    for(let i = 0; i < amount; i++){
        createPromise(i + 1, delay1).then(({ position , delay  })=>{
            (0, _notiflixNotifyAio.Notify).success(`✅ Fulfilled promise ${position} in ${delay}ms`);
        }).catch(({ position , delay  })=>{
            (0, _notiflixNotifyAio.Notify).failure(`✅ Fulfilled promise ${position} in ${delay}ms`);
        });
        delay1 += step;
    }
}
...
},{}]},["dDjGg","8tzFL"], "8tzFL", "parcelRequire1851")
